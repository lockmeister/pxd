...
// Commands
async function cmdNew(config, name, opts) {
    const res = await api(config, 'POST', '/id', { name: name || '' });
    if (!res.ok) {
        err(res.data.error || 'Unknown error', opts.json);
    }
    const tag = res.data;
    // Update cache
...
async function cmdShow(config, id, opts) {
    // Try cache first
    let tag = cacheGet(id);
    ...
    if (res.ok) {
        ...
        err(res.data.error || 'Not found', opts.json);
    }
    if (opts.json) {
        output(tag, true);
    }
...
async function cmdLink(config, id, type, url, opts) {
...
async function cmdSearch(config, query, opts) {
...
function cmdWork(id, opts = { json: false }) {
    if (!existsSync(CONFIG_DIR))
        mkdirSync(CONFIG_DIR, { recursive: true });
    if (id) {
...
async function cmdDelete(config, id, opts) {
...
async function main() {
    ...
    switch (cmd) {
        ...
        case 'claim':
            ...
            if (!claimRes.ok) {
                err(claimRes.data.error || 'Unknown error', jsonFlag);
            }
